...
public:
	...
	const short get_local_port(void) const;
	const short get_peer_port(void) const;

	const bool is_blocking(void) const;
	const bool set_blocking(const bool on = true);
...
inline const bool Socket::is_blocking(void) const
{
	return m_blocking;
}
