...
bool Socket::open(const char *host, const int port,
				  const sa_family_t family, const int type)
{
	return false;
}
...
const short Socket::get_local_port(void) const
{
	if (!isopen() || !m_local)
	{
		errno = EINVAL;
...
const short Socket::get_peer_port(void) const
{
	if (!isopen() || !m_peer)
	{
		errno = EINVAL;
...
const bool Socket::set_blocking(const bool on /* = true */)
{
	bool old = m_blocking;
	int  flags;
