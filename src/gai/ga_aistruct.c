...
/* include ga_aistruct2 */
	switch ((ai->ai_family = family)) {
		...
		case AF_INET6: {
			...
			if ( (sin6ptr = (struct sockaddr_in6 *) calloc(1, sizeof(struct sockaddr_in6))) == NULL)
				return(EAI_MEMORY);
#ifdef	HAVE_SOCKADDR_SA_LEN
...
/* *INDENT-OFF* */
			if (strlen((char *) addr) >= sizeof(unp->sun_path))
				return(EAI_SERVICE);
			if ( (unp = (struct sockaddr_un *)
				  calloc(1, sizeof(struct sockaddr_un))) == NULL)
				return(EAI_MEMORY);
/* *INDENT-ON* */
			unp->sun_family = AF_LOCAL;
			strcpy(unp->sun_path, (char *) addr);
#ifdef	HAVE_SOCKADDR_SA_LEN
